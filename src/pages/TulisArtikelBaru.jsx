...
import SuccessModal from '../components/SuccessModal'; // <-- Impor komponen modal
...
export default function TulisArtikelBaru() {
    ...
    const {
        ...
        isSubmitting, submitError, submitSuccess, handleSubmit,
    } = useArticleForm(user);
    ...
    useEffect(() => {
        if (!authLoading) {
            ...
            // Logika redirect atau alert bisa ditangani oleh ProtectedRoute/AdminRoute untuk konsistensi
        }
    ...
    return (
        ...
        >
            ...
            {/* --- DIUBAH DI SINI: Menambahkan SuccessModal --- */}
            <AnimatePresence>
                {submitSuccess && (
                    <SuccessModal 
                        message={submitSuccess} 
                        onClose={() => navigate('/artikel')} 
                    />
                )}
            </AnimatePresence>
            ...
            <div className="relative z-10 flex flex-col min-h-screen justify-between">
                ...
                <motion.main
                    className="flex-grow flex flex-col items-center justify-start pt-24 pb-16 px-4 gap-8"
                        ...
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg p-6 sm:p-8 md:p-10 w-full">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <textarea id="konten" rows="8" className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FF9F1C]" placeholder="Tulis konten artikel Anda di sini..." value={konten} onChange={(e) => setKonten(e.target.value)} required ></textarea>
                                    </div>
                                </div>
                                <div>
                                    <label htmlFor="gambarUrl" className="block text-gray-300 text-sm font-medium mb-2">URL Gambar (Opsional)</label>
                                    <div className="relative">
                                        <input type="url" id="gambarUrl" className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-3 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FF9F1C]" placeholder="https://example.com/gambar.jpg" value={gambarUrl} onChange={(e) => setGambarUrl(e.target.value)} />
                                ...
                                {/* --- DIUBAH DI SINI: Pesan teks sukses dihapus --- */}
                                
                                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className={`${glassButtonClasses} text-[#FF9F1C] hover:text-[#FF9F1C] disabled:opacity-50 disabled:cursor-not-allowed`} disabled={isSubmitting} >
                                ...
                                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="button" onClick={() => navigate('/artikel')} className={`${glassButtonClasses} text-gray-400 hover:text-gray-400 mt-2`} >
                                    <ChevronLeft size={20} /> Kembali ke Daftar Artikel
                                </motion.button>
                            </form>
                        </div>
                    </motion.div>
